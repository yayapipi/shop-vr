...
{	
    ...
    function OnInspectorGUI () {        
		...
		if((target as SunShafts).GetComponent.<Camera>())
			GUILayout.Label("Current GetComponent.<Camera>() mode: "+ (target as SunShafts).GetComponent.<Camera>().depthTextureMode, EditorStyles.miniBoldLabel);
		...
		if (newVal != oldVal) {
			if(newVal)
				(target as SunShafts).GetComponent.<Camera>().depthTextureMode |= DepthTextureMode.Depth;
			else
				(target as SunShafts).GetComponent.<Camera>().depthTextureMode &= ~DepthTextureMode.Depth;
		}
		*/
